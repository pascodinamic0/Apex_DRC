...
import { LayoutDashboard, FileText, Layers, Archive, LogOut, Users, HelpCircle, User, WifiOff, Wifi } from "lucide-react";
...
function Layout() {
  ...
  const syncQueuedDrafts = useCallback(async () => {
    const count = await replayDraftQueue(async (item: QueuedDraft) => {
      const { activities, narratives } = item.payload as { activities: unknown[]; narratives: Record<string, string> };
      await supabase.from("report_drafts").upsert(
        { report_id: item.reportId, payload: { activities, narratives }, updated_at: new Date().toISOString() },
